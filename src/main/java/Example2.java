...
public static String toHex(byte[] data) {
    ...
    public Example2(String host, int port) {
...
public ByteString wrapKey(ByteString kekKey, ByteString keyToWrap) throws IOException {
        // 16-byte IV for AES
        byte[] iv = new byte[16];
       // new SecureRandom().nextBytes(iv);
        ...
        // AES-CBC-PAD mechanism with IV
        Mechanism mech = Mechanism.newBuilder()
                .setMechanism(CKM_AES_CBC_PAD)
                .setParameterB(ByteString.copyFrom(iv))
                .build();
...
//*******************************************************************************
	...
	UnwrapKeyRequest request = UnwrapKeyRequest.newBuilder()
  		...
  		.setKeK(kek)
        	.setWrapped(wrappedBlob)
            	.putTemplate(CKA_KEY_TYPE,AttributeValue.newBuilder().setAttributeI(CKK_EC).build())
            	.putTemplate(CKA_SIGN, AttributeValue.newBuilder().setAttributeTF(true).build())
            	.putTemplate(CKA_EXTRACTABLE, AttributeValue.newBuilder().setAttributeTF(false).build())
            	.build();   // No template needed when original attributes must be preserved
    ...
    public static void main(String[] args) throws Exception {
        ...
        Example2 gen = new Example2("127.0.0.1", 9876);

        KeyPairBlob kp = gen.generateKeyPair(KeyType.ED25519);
        ByteString k =  gen.generateKey();
	System.out.println("Private Key: " + Example2.toHex(kp.priv.toByteArray()));
	System.out.println("Public Key: " + Example2.toHex(kp.pub.toByteArray()));
	System.out.println("SPKI: " + gen.parseSPKI(kp.pub));
        ByteString wrappedKey = gen.wrapKey(k,kp.priv);
        System.out.println();       
	System.out.println("Wrapped Key: " + Example2.toHex(wrappedKey.toByteArray()));
        UnwrappedKey unwrappedKey = gen.unwrapKey(k,wrappedKey);
        System.out.println();       
	System.out.println("Unwrapped Key:      " + Example2.toHex(unwrappedKey.key.toByteArray()));
	System.out.println("Unwrapped checksum: " + Example2.toHex(unwrappedKey.checksum.toByteArray()));
	System.out.println("SPKI: " + gen.parseSPKI(unwrappedKey.checksum));
//
     };
}
