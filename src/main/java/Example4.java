...
public static String toHex(byte[] data) {
    ...
    private static final long CKM_GENERIC_SECRET_KEY_GEN  =   0x00000350;
    private static final long  CKA_VENDOR_DEFINED  =   0x80000000;
    private static final long CKA_IBM_USE_AS_DATA = CKA_VENDOR_DEFINED +0x10008;
    private static final long CKK_GENERIC_SECRET = 0x00000010;
    private static final long  CKM_IBM_BTC_DERIVE     =      CKM_VENDOR_DEFINED + 0x00070001;
    private static final long   CKK_ECDSA     =      0x00000003;
    private static final int CK_IBM_BTC_SLIP0010_PRV2PRV      =   5;
    private static final int  CK_IBM_BTC_SLIP0010_PRV2PUB     =    6;
    ...
    public KeyPairBlob generateKeyPair(KeyType type) throws IOException {
        ...
        case ED25519:
            ...
            request = GenerateKeyPairRequest.newBuilder()
                    ...
                    // --- Public Key Template ---
                    .putPubKeyTemplate(CKA_EC_PARAMS, aB(OIDNamedCurveEd25519.getEncoded()))
...
public ByteString generateKey() throws IOException {
    ...
    Mechanism mechanism = Mechanism.newBuilder()
            .setMechanism(CKM_GENERIC_SECRET_KEY_GEN)
            .build();

    // Build AES key template (256-bit)
    GenerateKeyRequest request = GenerateKeyRequest.newBuilder()
            .setMech(mechanism)
            .putTemplate(CKA_KEY_TYPE, AttributeValue.newBuilder().setAttributeI(CKK_GENERIC_SECRET).build())
            ...
            .putTemplate(CKA_DERIVE, AttributeValue.newBuilder().setAttributeTF(true).build())
            .putTemplate(CKA_VERIFY, AttributeValue.newBuilder().setAttributeTF(true).build())
            .putTemplate(CKA_SIGN, AttributeValue.newBuilder().setAttributeTF(true).build())
            .putTemplate(CKA_IBM_USE_AS_DATA, AttributeValue.newBuilder().setAttributeTF(true).build())
...
public static class DerivedKey {
    public final ByteString key;
    public final ByteString chainCode;

    public DerivedKey(ByteString key, ByteString chainCode) {
        this.key = key;
        this.chainCode = chainCode;
    }
}

public DerivedKey slip10DeriveKey(
        int deriveType,          // CK_IBM_BTC_SLIP0010_MASTERK / PRV2PRV
        long childIndex,
        boolean hardened,
        ByteString baseKey,
        ByteString chainCode     // null for master
) {

    if (hardened) {
        childIndex += 0x80000000L;
    }

    // ------------------------------
    // Build BTC derive parameters
    // ------------------------------
    BTCDeriveParm.Builder btc = BTCDeriveParm.newBuilder()
            .setTypeValue(deriveType)
            .setChildKeyIndex(childIndex)
            .setVersion(1);

    if (chainCode != null) {
        btc.setChainCode(chainCode);
    }

    Mechanism mech = Mechanism.newBuilder()
            .setMechanism(CKM_IBM_BTC_DERIVE)
            .setBTCDeriveParameter(btc.build())
            .build();

    // ------------------------------
    // Base key
    // ------------------------------
    KeyBlob base = KeyBlob.newBuilder()
            .addKeyBlobs(baseKey)
            .build();
    ...
    try {
      ecParams = OIDNamedCurveSecp256k1.getEncoded();
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
    // ------------------------------
    // Template (matches Go/Python)
    // ------------------------------
    DeriveKeyRequest request = DeriveKeyRequest.newBuilder()
            .setMech(mech)
            .setBaseKey(base)
            .putTemplate(CKA_EC_PARAMS, aB(ecParams))
            .putTemplate(CKA_VERIFY, AttributeValue.newBuilder().setAttributeTF(true).build())
            .putTemplate(CKA_DERIVE, AttributeValue.newBuilder().setAttributeTF(true).build())
            .putTemplate(CKA_EXTRACTABLE, AttributeValue.newBuilder().setAttributeTF(false).build())
            .putTemplate(CKA_KEY_TYPE, AttributeValue.newBuilder().setAttributeI(CKK_ECDSA).build())
            .putTemplate(CKA_VALUE_LEN, AttributeValue.newBuilder().setAttributeI(32).build())
            .putTemplate(CKA_IBM_USE_AS_DATA, AttributeValue.newBuilder().setAttributeTF(true).build())
            .build();
    ...
    public static void main(String[] args) throws Exception {
        ...
        Example4 gen = new Example4("127.0.0.1", 9876);

        ByteString seed =  gen.generateKey();
